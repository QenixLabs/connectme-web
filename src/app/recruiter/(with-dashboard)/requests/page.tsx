...
export default function RecruiterRequestsPage() {
  ...
  const displayedSent = useMemo(
    () => filterByMessage(sortRequests(sent, sentSort)),
    [sent, sentSort, hasMessageOnly]
  );

  const displayedReceived = useMemo(
    () => sortRequests(pendingReceived, receivedSort),
    [pendingReceived, receivedSort]
  );
  ...
  const handleAccept = async (requestId: string) => {
    ...
    try {
      ...
      popup.show({
        ...
        description: "You can now message this talent.",
        variant: "success",
      });
      if (result?.conversation?._id) {
        router.push(`/recruiter/messages?conversationId=${result.conversation._id}`);
      }
    } catch (err) {
      popup.show({
  ...
  return (
    <div className="px-4 pt-6 pb-20 space-y-5">
      ...
      <RequestsStatsBar
        ...
        acceptedCount={acceptedSent.length}
        rejectedCount={rejectedSent.length}
        ...
        <TabsContent value="sent" className="mt-4 space-y-4">
          ...
          {displayedSent.length === 0 ? (
            <EmptyState
              ...
              description={
                hasMessageOnly
                  ? "No sent requests with messages."
        ...
        <TabsContent value="received" className="mt-4 space-y-4">
          ...
          {displayedReceived.length === 0 ? (
            <EmptyState
              ...
              description="When talent requests to connect, they'll appear here."
            />
          ) : (
            <div className="space-y-3">
              {displayedReceived.map((req) => (
                <RequestCard
                  key={req._id}
                  request={req}
