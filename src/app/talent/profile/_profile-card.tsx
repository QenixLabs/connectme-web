...
export function ProfileCard({ profile, actions, stats }: ProfileCardProps) {
  ...
  return (
    <>
      {/* Cover */}
      <div className="h-[72px] sm:h-24 relative overflow-hidden bg-gradient-to-br from-[#FDF3E0] via-[#FEF9F0] to-[#FCEFD6] border-b border-brand-muted/50">
          ...
          <div className="flex items-start gap-3.5 mb-3.5">
            ...
            <div className="flex-1 pt-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                <h2 className="text-[17px] font-medium text-text-primary truncate">{displayName}</h2>
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-light border border-brand-muted text-[11px] font-medium text-brand-hover">
                  <Star className="w-2.5 h-2.5 fill-brand text-brand" strokeWidth={1.5} />
                  Excellent
                </span>
              </div>
              {profile.username && (
                <p className="text-[13px] text-text-secondary mb-2">@{profile.username}</p>
              )}
