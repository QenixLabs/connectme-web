...
export default function AdminPortfolioPage() {
  ...
  const handleApprove = async (userId: string, itemId: string) => {
    ...
    try {
      ...
      const msg = getApiErrorMessage(err, "Failed to approve");
      setActionError(msg);
      toast.error(msg);
    } finally {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      const msg = getApiErrorMessage(err, "Failed to reject");
      setActionError(msg);
      toast.error(msg);
    } finally {
      setProcessingKey(null);
                    ...
                    >
                      <div className="shrink-0">
                        {talent.profile_photo ? (
                          ...
                          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-300 to-blue-500 flex items-center justify-center text-xs font-medium text-white">
                            {(talent.user_name || "U")
                              .charAt(0)
                              .toUpperCase()}
                          </div>
