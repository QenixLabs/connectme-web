...
export function CampaignWizard({ campaignId }: CampaignWizardProps) {
  ...
  const buildPayload = (
    values: CampaignWizardInput,
  ): Parameters<typeof campaignsApi.createCampaign>[0] => ({
    name: values.name,
    ...
    location:
      values.location?.city || values.location?.state
        ...
        : undefined,
    dates:
      values.dates?.start || values.dates?.end
        ? {
    ...
    questions: values.questions?.length
      ...
      : undefined,
    status:
      values.publishOption === "draft" || values.scheduled_publish_at
        ? "draft"
        : "active",
    visibility:
      values.publishOption === "invite_only" ? "invite_only" : "public",
    ...
    task:
      ...
      values.task?.nda_enabled
        ? {
            ...
            nda_text: values.task.nda_enabled
              ? values.task.nda_text || ""
              : undefined,
          }
        : undefined,
  });
    ...
    try {
      ...
      if (isEdit && campaignId) {
        await updateCampaign.mutateAsync({
          id: campaignId,
          payload: payload as Parameters<typeof campaignsApi.updateCampaign>[1],
        });
      } else {
      ...
      if (pendingTaskDoc && resultCampaignId) {
        try {
          await campaignsApi.uploadTaskDocument(
            resultCampaignId,
            pendingTaskDoc,
          );
        ...
        <div className="px-6 sm:px-8 py-6 space-y-6">
          ...
          >
            <RequirementsStep
              ...
              onPendingDocChange={setPendingTaskDoc}
            />
          </div>
          <div
            className={cn(
