...
export function MobileBottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-20 border-t bg-card/90 backdrop-blur-xl lg:hidden"
          ...
          <li key={item.label}>
            <button
              ...
              className={cn(
                ...
                active === item.label ? "text-accent" : "text-muted-foreground/45",
              )}
            >
              <item.icon width={20} height={20} />
              {item.badge ? (
                <span
                  className="absolute -top-0.5 right-2 grid h-4 min-w-4 place-items-center rounded-full bg-accent px-1 text-[10px] font-bold text-accent-foreground shadow-[0_0_8px_-3px_var(--accent)]"
