...
export function CreatePlanVersionSheet({ plan, onSuccess }: CreatePlanVersionSheetProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      popup.show({ title: "Plan version created", variant: "success" });
      setOpen(false);
      onSuccess();
      form.reset();
    } catch (err) {
      popup.show({ title: getApiErrorMessage(err, "Failed to create version"), variant: "error" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        ...
        <div className="mt-4 rounded-md bg-amber-50 border border-amber-200 p-3 flex gap-2 items-start">
