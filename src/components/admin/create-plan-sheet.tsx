...
import { z } from "zod";
import { Loader2, Plus, X, Check, PlusCircle } from "lucide-react";
...
import { plansApi, type CreatePlanInput } from "@/lib/api/plans";
import { usePopup } from "@/hooks/use-popup";
import { getApiErrorMessage } from "@/lib/formatters";
import { PermissionSelector } from "./permission-selector";

const ROLES = ["talent", "recruiter", "both"] as const;
...
export function CreatePlanSheet({ onSuccess }: CreatePlanSheetProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      const payload: CreatePlanInput = {
        ...
        family_key: values.family_key?.trim().toLowerCase() || undefined,
      ...
      popup.show({ title: "Plan created", variant: "success" });
      setOpen(false);
      onSuccess();
      form.reset();
    } catch (err) {
      popup.show({ title: getApiErrorMessage(err, "Failed to create plan"), variant: "error" });
    } finally {
      setIsSaving(false);
    }
  };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 space-y-5">
                ...
                <FormItem className="space-y-1.5">
                  ...
                  <FormControl>
                    <Input
                      ...
                      onBlur={(e) => {
                        field.onBlur();
                        const familyKey = form.getValues("family_key");
                        if (!familyKey && e.target.value) {
                          form.setValue("family_key", e.target.value.trim().toLowerCase());
                        }
                      }}
                    />
                  </FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-1.5">
                  <FormLabel className="text-xs">Family Key</FormLabel>
                  <FormControl>
                    <Input {...field} className="h-9 text-sm" />
                  </FormControl>
                  <p className="text-[10px] text-muted-foreground">
                    Groups plan versions together. Defaults to plan key if empty.
                  </p>
                  <FormMessage />
                </FormItem>
              )}
